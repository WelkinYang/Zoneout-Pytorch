...
inputs = torch.randn([batch_size, max_time, feature_dims])
...
outputs = torch.zeros([batch_size, max_time, hidden_size * 2])
forward_h = torch.zeros(batch_size, hidden_size)
forward_c = torch.zeros(batch_size, hidden_size)
forward_state = (forward_h, forward_c)
backward_h = torch.zeros(batch_size, hidden_size)
backward_c = torch.zeros(batch_size, hidden_size)
backward_state = (backward_h, backward_c)

for i in range(max_time):
    forward_input = inputs[:, i, :]
    backward_input = inputs[:, max_time-(i+1), :]
    forward_output, backward_output, forward_new_state, backward_new_state = zoneout_rnn(forward_input, backward_input, forward_state, backward_state)
